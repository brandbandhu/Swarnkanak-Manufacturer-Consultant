  ...
  head: () => ({
    meta: [
      { title: "Agricultural Machinery Shop | Swarnkanak Implements" },
      {
        name: "description",
        content:
          "Browse rotavators, harrows, ploughs, seed drills, sprayers, balers, mulchers and trailers with prices, specifications and instant online ordering.",
      },
      { property: "og:title", content: "Agricultural Machinery Shop | Swarnkanak" },
      {
        property: "og:description",
        content:
          "30+ agricultural implements with filters for category, price, tractor HP and availability.",
      },
    ],
  }),
...
function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.q) {
      ...
      list = list.filter((p) =>
        (p.name + p.sku + p.category + p.subcategory).toLowerCase().includes(q),
      );
    }
  ...
  const chips = [
    ...
    search.category && {
      ...
      clear: () => setSearch({ category: undefined, sub: undefined }),
    },
  ...
  return (
    <SiteLayout>
      <PageHeading
        title={search.category ?? "All Agricultural Machinery"}
        subtitle={
          activeCat?.blurb ??
          "Explore our complete range of tractor implements with transparent pricing, specifications and online ordering."
        }
      />
      <Breadcrumbs
        items={[
          ...
          <Card>
            <CardContent className="space-y-4 p-5">
                  ...
                  {CATEGORIES.map((c) => (
                    <li key={c.id}>
                      <button
                        ...
                        onClick={() => setSearch({ category: c.name, sub: undefined })}
                      >
                        {c.name}
                      </button>
                      {search.category === c.name && (
              ...
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={inStockOnly} onCheckedChange={(v) => setInStockOnly(!!v)} /> In
                stock only
              </label>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={offersOnly} onCheckedChange={(v) => setOffersOnly(!!v)} />{" "}
                Discounted products
              </label>
              <Button
                variant="outline"
                ...
                onClick={() => {
                  ...
                  setOffersOnly(false);
                  setSearch({ q: undefined, category: undefined, sub: undefined });
                }}
              >
                Clear all filters
              </Button>
            ...
            <div className="mt-8 flex justify-center gap-2">
              {Array.from({ length: pages }, (_, i) => (
                <Button
                  ...
                  onClick={() =>
                    navigate({ search: (prev: ProductSearch) => ({ ...prev, page: i + 1 }) })
                  }
                >
                  {i + 1}
                </Button>
